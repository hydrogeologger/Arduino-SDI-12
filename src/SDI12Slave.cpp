...
sdi12timer_t SDI12Slave::_previous_TCNT = 0;  // previous RX transition in micros
...
void SDI12Slave::receiveISR(void) {
    if (waiting_for_break_ || waiting_for_mark_) {
        ...
        if (waiting_for_break_) {
            if (pinLevel == HIGH) {
                return;
            } else if (SDI12_TCNT_TO_MICROSECONDS(dt) >= SDI12SLAVE_LINE_BREAK_MICROS) {
                waiting_for_break_ = false;
            }
        } else if (waiting_for_mark_ && (pinLevel == HIGH) &&
                (SDI12_TCNT_TO_MICROSECONDS(dt) >= SDI12SLAVE_LINE_MARK_MICROS)) {
            waiting_for_mark_ = false;
        }

        // Serial.print(" : "); Serial.print(waiting_for_break_);
        // Serial.print(" : "); Serial.print(SDI12_TCNT_TO_MICROSECONDS(dt) >= SDI12SLAVE_LINE_BREAK_MICROS);
        // Serial.print(" : "); Serial.print(waiting_for_mark_);
        // Serial.print(" : "); Serial.print(SDI12_TCNT_TO_MICROSECONDS(dt) >= SDI12SLAVE_LINE_MARK_MICROS);
        // Serial.println("");
